"""Offline trainer: Domain Randomized Frame-Stacked SAC for OBELIX Phase 4 (Test Phase)."""
...
class Critic(nn.Module):
    def __init__(self, in_dim=IN_DIM, n_actions=5, hidden_dim=128):
        super().__init__()
        self.q1 = nn.Sequential(nn.Linear(in_dim, hidden_dim),nn.ReLU(),nn.Linear(hidden_dim, hidden_dim),nn.ReLU(),nn.Linear(hidden_dim, n_actions))
        self.q2 = nn.Sequential(nn.Linear(in_dim, hidden_dim),nn.ReLU(),nn.Linear(hidden_dim, hidden_dim),nn.ReLU(),nn.Linear(hidden_dim, n_actions))
...
def main():
    ...
    ap.add_argument("--episodes", type=int, default=4000) # Increased for multi-domain learning
    ...
    ap.add_argument("--warmup", type=int, default=3000) # More warmup for the complex buffer
    ...
    for ep in range(args.episodes):
        # DOMAIN RANDOMIZATION: Train across all rulesets simultaneously
        rand_difficulty = random.choice([0, 2, 3]) 
        rand_wall = random.choice([True, False])

        env = OBELIX(
            scaling_factor=args.scaling_factor,
            arena_size=args.arena_size,
            max_steps=args.max_steps,
            wall_obstacles=rand_wall,
        ...
        if (ep + 1) % 50 == 0:
            print(f"Episode {ep+1}/{args.episodes} | Return: {ep_ret:.1f} | Diff: {rand_difficulty} | Wall: {rand_wall}")

    torch.save(actor.state_dict(), args.out)
    print("Saved generalist weights to:", args.out)
