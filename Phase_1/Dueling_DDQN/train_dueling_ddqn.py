...
class DuelingDQN(nn.Module):
    def __init__(self, in_dim=18, n_actions=5):
        ...
        self.value_stream = nn.Sequential(nn.Linear(64, 64),nn.ReLU(),nn.Linear(64, 1)) #State-value stream V(s)
        self.advantage_stream = nn.Sequential(nn.Linear(64, 64),nn.ReLU(),nn.Linear(64, n_actions)) #Action-advantage stream A(s,a)

    def forward(self, x):
        features=self.feature(x)
        values=self.value_stream(features)
        advantages=self.advantage_stream(features)
        qvals=values+(advantages-advantages.mean(dim=1, keepdim=True)) #Q(s,a)=V(s)+(A(s,a)-mean(A(s,a)))
        return qvals
...
def main():
    ...
    ap.add_argument("--tau", type=float, default=0.1) #Added for Polyak Averaging
    ...
    for ep in range(args.episodes):
        env = OBELIX(scaling_factor=args.scaling_factor,arena_size=args.arena_size,max_steps=args.max_steps,wall_obstacles=args.wall_obstacles,difficulty=args.difficulty,box_speed=args.box_speed,seed=args.seed + ep,)
